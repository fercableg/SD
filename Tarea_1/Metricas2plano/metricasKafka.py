...
def get_lag(group):
    ...
    for line in lines:
        ...
        if len(parts) >= 6:
            try:
                total += int(parts[5])   # la columna LAG
            except ValueError:
                pass
    return total
...
def guardar_csv(timestamp, lag_principal, lag_reintentos):
    ...
    with open(ARCHIVO_CSV, mode='a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([timestamp, lag_principal, lag_reintentos])
...
def iniciar_monitoreo():
    ...
    print(f"Monitoreando lag cada {INTERVALO} segundos. Presiona Ctrl+C para detener.")
    try:
        while True:
            ts = time.time()
            ...
            guardar_csv(ts, lag_principal, lag_reintentos)
            time.sleep(INTERVALO)
    except KeyboardInterrupt:
        print("\nMonitoreo detenido. Datos guardados en", ARCHIVO_CSV)
